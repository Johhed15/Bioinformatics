...
Here we use clustal() and mafft() to align the genetic sequences and ensure the sequences are properly aligned for phylogenetic analysis.
...
Calculating distance for various models (K80, F84, TN93, GG95). The dist.dna() function computes pairwise genetic distances from the aligned sequences.
...
Identify the Chamaea sequence as reference which we create a neighbor-joining tree using nj() function and root which is rooted to that sequence. Then we perform a bootstrap analysis too assess the tree reliability. We then create tip labels, plot the tree and write it.
...
Reads the written tree again, drops species that are not Curruca or Sylvia, uniy those species and make sure spelling is correct, then extract all the aforementioned information regarding species of migration distance & behavior + geographical range.
